    ...
    def perform_create(self, serializer):
        try:
            with transaction.atomic():
                ...
                # update status of first algorithm
                ...
                # update status of second algorithm
...
class StopABTestView(views.APIView):
    def post(self, request,ab_test_id, format=None):
        ...
        try:
            ...
            all_responses_1 = MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithm_1, created_at__gt = ab_test.created_at,
            created_at__lt=date_now).count()
            correct_responses_1 = MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithm_1, created_at__gt = ab_test.created_at,
            ...
            all_responses_2=MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithms_2,
            created_at__gt= ab_test.created_at, created_at__lt = date_now).count()
            correct_responses_2 = MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithm_2, created_at__gt = ab_test.created_at, 
            created_at__lt = date_now, response=F('feedback')).count()
            accuracy_2 = correct_responses_2 / float(all_responses_2)
            print(all_responses_2, correct_responses_2, accuracy_2)
            # select algorithm with the highest accuracy
            ...
            status_1.save()
            deactivate_other_statuses(status_1)
            # update second algorithm status
